...
public class LogicImageDialog extends BaseDialog {
    ...
    Schematic review = new Schematic(Seq.with(), new StringMap(), 1, 1);
    ...
    public LogicImageDialog() {
        ...
        for (Block block : Vars.content.blocks().select(b -> b instanceof LogicBlock)) {
            logicBlocks.add(new Selection<>((LogicBlock) block, new TextureRegionDrawable(block.fullIcon), block.description));
        }
        ...
        for (Block block : Vars.content.blocks().select(b -> b instanceof LogicDisplay)) {
            logicDisplays.add(new Selection<>((LogicDisplay) block, new TextureRegionDrawable(block.fullIcon), block.description));
        }
        ...
        list.add(new SelectionSetting<>("色块化算法",
                Seq.with(
                        new Selection<>(Compressor.quadtree, Icon.move, "四叉树分割"),
        ...
        list.add(new ToggleCheckSetting("生成并保存", () -> {
            ...
            Compressors.compressor(logicDisplay, logicBlock, widthAmount, heightAmount, compressor);
    ...
    void refreshReviewImage(){
        ...
        TextureRegion region = new TextureRegion(new Texture(image));
        scale = imageSize / Math.max(region.width, region.height);
        w = region.width * scale;
        h = region.height * scale;
        reviewTable.image(region).size(w, h).pad(15f).get();

        TextureRegion regionn = new TextureRegion(new Texture(afterScaling));
        scale = imageSize / Math.max(regionn.width, regionn.height);
        w = regionn.width * scale;
        h = regionn.height * scale;
        reviewTable.image(regionn).size(w, h).pad(15f).get();

        TextureRegion regionnn = new TextureRegion(new Texture(afterCompressor));
        scale = imageSize / Math.max(regionnn.width, regionnn.height);
        w = regionnn.width * scale;
        h = regionnn.height * scale;
        reviewTable.image(regionnn).size(w, h).pad(15f).get();
    ...
    static class SelectionSetting<T> extends Setting{
        ...
        public void add(SettingsTable table) {
            ...
            Table is = new Table(b -> items.each(t -> {
                ...
                }).size(48).checked(ib -> holder.get() == t.item);
            }));
